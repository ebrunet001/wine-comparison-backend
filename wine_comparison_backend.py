...
Version optimisée avec fuzzy matching amélioré
"""
...
app = Flask(__name__)
# Configuration CORS plus permissive pour autoriser file://
CORS(app, resources={
    r"/api/*": {
        "origins": ["*"],
...
def compare_files():
    ...
    try:
        ...
        ld_path = os.path.join(UPLOAD_FOLDER, secure_filename(ld_file.filename))
        gs_path = os.path.join(UPLOAD_FOLDER, secure_filename(gs_file.filename))
        ...
        logger.info("Colonnes Livre de cave:")
        for i, col in enumerate(livredecave_df.columns):
            sample = livredecave_df.iloc[0][col] if len(livredecave_df) > 0 else 'N/A'
            logger.info(f"  Col {i}: '{col}' - Exemple: {str(sample)[:50]}")
        
        logger.info("Colonnes Google Sheet:")
        for i, col in enumerate(googlesheet_df.columns):
            sample = googlesheet_df.iloc[0][col] if len(googlesheet_df) > 0 else 'N/A'
            logger.info(f"  Col {i}: '{col}' - Exemple: {str(sample)[:50]}")
        
        # Recherche spécifique des vins mentionnés
        logger.info("\nRECHERCHE SPÉCIFIQUE DES VINS MENTIONNÉS:")
        
        # Chercher Roulot dans Google Sheet
        for idx, row in googlesheet_df.iterrows():
            wine_name = str(row.iloc[0]).lower() if pd.notna(row.iloc[0]) else ''
            if 'roulot' in wine_name or 'desjourneys' in wine_name:
                logger.warning(f"TROUVÉ dans Google Sheet ligne {idx+2}: {row.iloc[0]}")
                if len(row) > 2:
                    logger.warning(f"  Millésime: {row.iloc[2]}, Contenance: {row.iloc[3] if len(row) > 3 else 'N/A'}")
        
        # Chercher dans Livre de Cave
        for idx, row in livredecave_df.iterrows():
            wine_parts = []
            for col_idx in [2, 4, 5, 6]:  # C, E, F, G
                if col_idx < len(row) and pd.notna(row.iloc[col_idx]):
                    wine_parts.append(str(row.iloc[col_idx]))
            wine_name = ' '.join(wine_parts).lower()
            if 'roulot' in wine_name or 'desjourneys' in wine_name:
                logger.warning(f"TROUVÉ dans Livre de Cave ligne {idx+2}: {' '.join(wine_parts)}")
                if len(row) > 7:
                    logger.warning(f"  Millésime: {row.iloc[7]}, Contenance: {row.iloc[8] if len(row) > 8 else 'N/A'}")
...
if __name__ == '__main__':
    print("🍷 Serveur de comparaison de caves - Version Optimisée")
    print("=" * 60)
    print("Améliorations:")
